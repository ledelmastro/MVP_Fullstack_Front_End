document.addEventListener('DOMContentLoaded', () => {
    ...
    //--------------------------------------------------------------------------------------
    // Função assíncrona para selecionar produto
    async function selectProduct(product) {
        ...
        if (selectedProductsCount[product.id]) {
            ...
            let response = await fetch('http://127.0.0.1:5000/selected-products');
    ...
    function changeQuantity(product, amount, productDiv) {
        ...
        // Atualiza o preço total do produto
        updateProductTotalPrice(productDiv, product);
        // Atualiza o produto no servidor após a mudança de quantidade
    ...
    // Função para para atualizar um produto no servidor existente do servidor via requisição PUT
    async function updateProductOnServer(product) {
        try {
            ...
            console.log('Produto atualizado com sucesso no servidor!');
        } catch (error) {
    ...
    async function removeProduct(element, productId) {
        const selectedProduct = products.find(product => product.id === productId);
        if (confirm(`Tem certeza que deseja remover o produto ${selectedProduct.name} ?`)) {
            //(`Produto` + selecionado + `Removido!`)
            if (selectedProduct) {
                // Exiba o nome do produto em um alert
                alert(`${selectedProduct.name} removido com sucesso.`);
            } else {
            ...
            fetch('http://127.0.0.1:5000/' + productId, {
                method: 'DELETE',
            })
                .then(response => {
                    if (!response.ok) {
                    ...
                    console.log('Produto removido com sucesso do servidor!');
                    // Recarregue a página ou atualize a lista de produtos na interface
                    // location.reload(); //Exemplo: Recarrega a página
                })
